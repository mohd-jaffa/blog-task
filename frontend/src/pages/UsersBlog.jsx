...
import { setUsersName, usersBlogs } from "../slices/adminSlice";
import { deleteBlog, setEditId } from "../slices/blogSlice";

export default function UsersBlog() {
    const { id } = useParams();
    const dispatch = useDispatch();
    const { data, loading, error, usersName } = useSelector(
        (state) => state.admin
    );

    useEffect(() => {
        dispatch(usersBlogs(id));
        return () => {
    ...
    const handleDelete = async (postId) => {
        ...
        if (isConfirm) {
            try {
                await dispatch(deleteBlog(postId)).unwrap();
                dispatch(usersBlogs(id));
            } catch (err) {
    ...
    return (
        <div>
            <h1 className="text-3xl">Blogs of {usersName}</h1>
